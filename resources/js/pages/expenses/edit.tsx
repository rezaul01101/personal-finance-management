...
import expenseAttachments from '@/routes/expenses/attachments';
import type {
    Account,
    BudgetCategory,
    Expense,
...
export default function ExpensesEdit({
    ...
    function submit() {
        form.put(expenses.update.url(expense.id), { forceFormData: true });
    }

    function removeAttachment(attachmentId: number) {
        router.delete(
            expenseAttachments.destroy.url({
                expense: expense.id,
...
ExpensesEdit.layout = (props: { expense: Expense }) => ({
    breadcrumbs: [
        { title: 'Expenses', href: expenses.index() },
        { title: 'Edit Expense', href: expenses.edit(props.expense.id) },
    ],
});
