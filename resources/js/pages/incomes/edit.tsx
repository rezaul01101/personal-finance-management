...
import type { Account, Income, IncomeSource } from '@/types/finance';
...
export default function IncomesEdit({
    ...
    const form = useForm<{
        ...
        account_id: String(income.account_id),
        received_on: income.received_on,
        note: income.note ?? '',
    });

    function submit() {
        form.put(incomes.update.url(income.id));
    }
...
IncomesEdit.layout = (props: { income: Income }) => ({
    breadcrumbs: [
        { title: 'Income', href: incomes.index() },
        { title: 'Edit Income', href: incomes.edit(props.income.id) },
    ],
});
